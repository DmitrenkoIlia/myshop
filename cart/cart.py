...
class Cart(object):
    ...
    def add(self, product, quantity=1, update_quantity=False):
        ...
        # Определяем итоговое количество, которое будет в корзине
        if update_quantity:
            final_quantity = quantity
        else:
            final_quantity = self.cart[product_id]['quantity'] + quantity

        # ПРОВЕРКА: Нельзя добавить больше, чем есть на складе
        if final_quantity > product.stock:
            # Устанавливаем количество равное максимальному доступному
